...
async function ensureCanManageStaff(orgId: string, userId: string) {
  ...
  if (!canManage) {
    return { error: { status: 403, message: "No tienes permisos para gestionar personal" } };
  }
...
El enlace expira el ${newExpiresAt.toLocaleDateString()}`,
      ...
      return NextResponse.json(
        {
          error: "No se pudo reenviar el correo de invitacion. Intenta nuevamente.",
        },
        { status: 502 }
      );
    }
