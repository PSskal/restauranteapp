...
async function assertWriter(orgId: string) {
  ...
  if (!membership && !isOwner) {
    return {
      error: "No tienes permisos para editar zonas",
      status: 403 as const,
    };
  }
...
export async function PATCH(
  request: NextRequest,
  { params }: { params: Promise<{ orgId: string; zoneId: string }> }
) {
  try {
    const { orgId, zoneId } = await params;
    ...
    log.error("zones.update.exception", error);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    // Las mesas quedan con zoneId NULL (onDelete: SetNull)
    await prisma.zone.delete({ where: { id: zoneId } });

    return NextResponse.json({ ok: true });
  } catch (error) {
    log.error("zones.delete.exception", error);
    return NextResponse.json(
      { error: "Error interno del servidor" },
      { status: 500 }
    );
