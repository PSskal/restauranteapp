...
export function ReportsDashboard() {
  ...
  const statusEntries = useMemo(() => {
    ...
    return (Object.entries(data.statusCounts) as Array<
  ...
  const rangeSummary = data
    ? `${formatShortDate(data.range.from)} - ${formatShortDate(
        data.range.to
      )}`
    : "Selecciona un rango";
                        ...
                        return (
                          <div className="rounded-lg border bg-background p-3 text-xs shadow-sm">
                            ...
                            <p className="text-muted-foreground">
                              Ingresos:{" "}
                              {formatCurrency(Math.round(point.ingresos * 100))}
                            </p>
                            <p className="text-muted-foreground">
                            ...
                            <p className="text-muted-foreground">
                              Ticket medio:{" "}
                              {formatCurrency(Math.round(point.ticket * 100))}
                            </p>
                          </div>
                        );
                      }}
                    ...
                    return (
                      <div key={method} className="space-y-2">
                        <div className="flex items-center justify-between text-sm font-medium">
                          <span>{PAYMENT_LABELS[method as PaymentMethodKey]}</span>
